...
public sealed partial class HotkeyListener : IHostedService, IDisposable
{
    ...
    /// <summary>
    /// Temporarily unregisters all global hotkeys (keyboard AND mouse). Use case: Settings
    /// dialog is open and the user wants to rebind one of the already-bound inputs — without
    /// this, <c>RegisterHotKey</c> / <c>WH_MOUSE_LL</c> intercept the press and the dialog
    /// never sees the KeyDown / PointerPressed event. Pair with <see cref="ResumeAsync"/>.
    /// </summary>
    ...
    /// <summary>
    /// Reverses <see cref="SuspendAsync"/>. Re-reads the current bindings from the store
    /// (which may have changed during the suspension) and re-registers both monitors.
    /// </summary>
