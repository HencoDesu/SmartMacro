...
public sealed partial class Orchestrator : IHostedService, IMacroRunner, IDisposable
{
    ...
    public async Task RunAsync(string macroName, IntPtr? contextWindow, string? singleFlightKey)
    {
        ...
        // Ссылки на побудку, которые держит ПРОГОН, — вторая половина хука со Scope: Run.
        // Заводятся здесь и закрываются в finally на ЛЮБОМ пути выхода: окно, разбуженное на весь
        // прогон, обязано заснуть вместе с ним, чем бы прогон ни кончился. Пустая корзина (обычный
        // случай — все хуки Scope: Action) не стоит ничего.
        var hooks = new MacroRunHooks(_windows);
        try
        {
            var context = new MacroRunContext
            {
                ...
                // Ставится ЗДЕСЬ и ровно один раз на прогон — рядом с шаблонами и под-макросами,
                // по той же причине: «весь прогон» есть свойство прогона, и дочерние обходы
                // наследуют ту же корзину по ссылке.
                Hooks = hooks,
        ...
        {
            // СНАЧАЛА усыпляем окна, и только потом снимаем прогон с учёта. Порядок значим:
            // StopAsync ниже ждёт именно снятия с учёта, и его обещание «ни один прогон не бросают
            // посреди активации, оставив клиент разбуженным» держится на том, что к этому моменту
            // ссылки прогона уже закрыты.
            await hooks.DisposeAsync().ConfigureAwait(false);
