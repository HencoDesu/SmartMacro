...
/// пять строк, которые можно прочитать до того, как что-то сломается, дешевле, чем каждый раз
/// расследовать заново.
///
/// Шестую проверку — время ответа канала — делает панель: демон не может честно измерить время
/// ответа самому себе.
///
/// <b>Все проверки безопасны и ничего не меняют.</b> Самая «активная» шлёт окнам <c>WM_NULL</c>.
/// Кнопку жмут, когда что-то уже не работает, и диагностика, у которой есть побочные эффекты,
/// в этот момент — последнее, что нужно.
/// </summary>
