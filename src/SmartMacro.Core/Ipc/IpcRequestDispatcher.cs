...
public sealed partial class IpcRequestDispatcher
{
        ...
        CancellationToken cancellationToken)
    {
        switch (request.Type)
        {
            ...
            case IpcMessageTypes.RunMacro:
            {
                ...
                if (entry is null || ChangedSinceItWasRead(entry))
                {
                    _macros.Refresh();
    ...
    /// <summary>
    /// Переписали ли бандл уже ПОСЛЕ того, как хранилище сняло свой снимок.
    ///
    /// <b>Спрашиваем у файловой системы одну отметку времени, а не перечитываем папку.</b>
    /// Безусловный <c>Refresh</c> был бы проще, но он разбирает КАЖДЫЙ бандл в <c>macros/</c> на
    /// каждое нажатие ▸ — то есть платит полным чтением библиотеки за случай, который бывает раз в
    /// сеанс. Здесь одна проверка атрибутов файла: ни открытия архива, ни разбора JSON.
    ///
    /// <b>Сравнение точное, окна больше нет.</b> Раньше здесь стоял ВОЗРАСТ файла с допуском в две
    /// секунды: подозрительным считался любой бандл, записанный только что. Оценка работала, но
    /// была именно оценкой — она не отличала «файл записан после того, как мы его прочли» от
    /// «файл записан только что, и хранилище уже успело его перечитать», и каждое ▸ по свежесохранённому
    /// макросу стоило лишнего чтения папки. Теперь запись библиотеки несёт отметку файла НА МОМЕНТ
    /// ЧТЕНИЯ (<see cref="MacroLibraryEntry.FileWrittenUtc"/>), и вопрос задаётся ровно тот, что
    /// нужен: изменилась ли она с тех пор. Константы не требуется, а «дата в будущем» (файл
    /// принесли с машины со спешащими часами) обрабатывается сама собой — она попросту не равна
    /// сохранённой.
    ///
    /// ⚠️ Сверять с датой правки из ПАСПОРТА по-прежнему нельзя: паспорт штампуется до
    /// сериализации, то есть эти две отметки ставятся в разные мгновения одной записи, и сверке
    /// снова потребовался бы допуск.
    /// </summary>
    private static bool ChangedSinceItWasRead(MacroLibraryEntry entry)
    {
        try
        {
            // Файла нет — GetLastWriteTimeUtc отдаёт 1601 год, что заведомо не равно сохранённой
            // отметке, и папка будет перечитана. Это верный ответ: бандл удалили, снимок отстал.
            return File.GetLastWriteTimeUtc(entry.Path) != entry.FileWrittenUtc;
        }
