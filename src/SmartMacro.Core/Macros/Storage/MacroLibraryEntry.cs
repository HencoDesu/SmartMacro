...
/// <param name="FileWrittenUtc">
/// Отметка последней записи файла, снятая ТОГДА, КОГДА ЕГО ЧИТАЛИ.
///
/// Нужна ровно для одного вопроса: «изменился ли бандл с тех пор, как мы взяли его в снимок».
/// Спрашивает его <c>IpcRequestDispatcher</c> на каждое ▸, чтобы не прогнать старый граф, пока
/// наблюдатель гасит дребезг. Раньше там стоял ВОЗРАСТ файла с окном в две секунды — оценка,
/// которая по построению не могла быть точной: она называла подозрительным любой файл, записанный
/// недавно, включая тот, который хранилище уже перечитало. Сравнение отметки с самой собой точнее
/// и константы не требует.
///
/// ⚠️ Сравнивать с датой правки из ПАСПОРТА по-прежнему нельзя, и это отдельная мысль: паспорт
/// штампуется до сериализации, то есть двумя разными мгновениями одной записи, и сверке
/// потребовался бы допуск — то самое окно, от которого здесь избавились.
/// </param>
