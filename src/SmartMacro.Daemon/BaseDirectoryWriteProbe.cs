...
/// <summary>
/// Проба пера в каталоге, где лежит демон: можно ли туда писать.
///
/// Раскладка портативная — всё состояние демона лежит РЯДОМ С EXE, а не в
/// <c>%LOCALAPPDATA%</c>: <c>macros\*.json</c>, <c>logs\smartmacro-*.log</c>, <c>debug\</c> со
/// снимками. Один путь на всё, папку можно перенести или скопировать целиком. Плата — вся
/// портативность держится на том, что распаковали туда, куда разрешено писать: в
/// <c>C:\Program Files</c> или на сетевой шаре без прав не запишется НИЧЕГО, причём молча.
///
...
public static class BaseDirectoryWriteProbe
{
    ...
    public static string DescribeFailure(string directory, string? failure) =>
        ...
        "SmartMacro хранит макросы, журналы и снимки экрана рядом со своими исполняемыми " +
        "файлами. Распакуйте папку туда, куда можно писать (например, в свой каталог " +
