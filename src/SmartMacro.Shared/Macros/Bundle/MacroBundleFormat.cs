...
/// и git умеет считать по такому файлу дельту, а не хранить каждую версию целиком.
///
/// ⚠️ <b>«Побочно исчезает и класс зип-бомб» здесь было написано, и это неправда для ЧУЖОГО
/// бандла.</b> Свойство «распакованный размер равен размеру файла» обеспечивает ПИСАТЕЛЬ, а импорт
/// — это копирование файла: содержимое намеренно не разбирается, иначе прогон через сегодняшнего
/// писателя потерял бы всё, чего сегодняшний формат не знает. Читатель сжатия не запрещает и
/// запрещать не должен, поэтому у него есть пределы — см. <see cref="MaxEntryBytes"/> и
...
public static class MacroBundleFormat
{
    ...
    /// <summary>
    /// Предел на РАСПАКОВАННЫЙ размер ОДНОЙ записи бандла — 32 МиБ.
    ///
    /// <b>Ориентир взят от настоящих макросов, а не от круглого числа.</b> Одиннадцать вырезанных
    /// вручную PNG с именами классов весят десятки килобайт каждый; самое тяжёлое, что вообще
    /// может законно попасть в бандл, — полнокадровый шаблон с экрана 3840×2160, а это единицы
    /// мегабайт. Тридцать два — троекратный запас над этим, и одновременно потолок, который
    /// демон переживает: неудачная запись стоит одного отказа, а не нехватки памяти в резидентном
    /// движке со всеми хоткеями и прогонами.
    ///
    /// ⚠️ Предел живёт у ЧИТАТЕЛЯ, а не у импорта, и это не мелочь: импорт намеренно не
    /// разбирает файл (см. <c>MacroBundleFolder.Import</c>), так что проверять надо там, где
    /// принимают решение распаковать.
    /// </summary>
    public const long MaxEntryBytes = 32L * 1024 * 1024;

    /// <summary>
    /// Предел на СУММАРНЫЙ распакованный размер бандла — 128 МиБ.
    ///
    /// Один предел на запись зип-бомбу не закрывает: тысяча записей по мегабайту тоже кладёт
    /// демон. Реальный макрос с распознаванием класса и парой полнокадровых шаблонов не доходит
    /// и до тридцати мегабайт, так что запас здесь примерно шестикратный.
    ///
    /// Проверяется по оглавлению архива (дёшево, ничего не распаковывая) — это и есть вердикт,
    /// который видит пользователь в строке библиотеки. Второй рубеж — само чтение записи: оно
    /// отказывается отдать байты, если распаковка даёт не тот размер, что объявлен, потому что
    /// оглавление пишет тот же, кто прислал файл.
    /// </summary>
