...
public enum MacroBundleFault
{
    ...
    /// <summary>
    /// Запись разбирается, но называет тип ноды или триггера, которого эта сборка не знает.
    ///
    /// НЕ <see cref="Malformed"/> и НЕ <see cref="UnsupportedVersion"/>, и это ровно та развилка,
    /// ради которой всё перечисление и заведено. «Повреждён» отправляет чинить файл, который цел;
    /// «сделан другой версией формата» — вердикт ПОЛЯ ВЕРСИИ, а оно здесь своё, и подменять его
    /// догадкой значило бы соврать тем самым способом, ради недопущения которого поле заведено.
    /// Честный ответ один: назвать незнакомый тип так, как он написан в файле.
    /// </summary>
    UnknownType,
    ...
    /// <summary>
    /// Распакованный бандл не влезает в пределы <see cref="MacroBundleFormat.MaxEntryBytes"/> /
    /// <see cref="MacroBundleFormat.MaxBundleBytes"/>.
    ///
    /// Отдельное значение, а не «повреждён», и причина ровно та же, что у
    /// <see cref="UnsupportedVersion"/>: файл может быть совершенно целым, и действие
    /// пользователя тут другое — не «почините бандл», а «в нём лежит что-то, чего там быть не
    /// должно». Появилось вместе с проверкой размера: свойство «зип-бомб не бывает» обеспечивал
    /// ПИСАТЕЛЬ (без сжатия распакованный размер равен размеру файла), а импорт — это копирование
    /// файла, и на чужой бандл писательская гарантия не распространяется.
    /// </summary>
    TooLarge,
}
...
/// <param name="Bytes">
/// РАСПАКОВАННЫЙ размер записи — то есть вес самого PNG: столько он займёт, если его вытащить, и
/// столько же будет стоить его декодирование в превью (потолок превью сверяется именно с этим
/// числом).
///
/// ⚠️ Здесь было написано «он же размер файла: бандл не сжимается», и это перестало быть правдой
/// в тот день, когда появился импорт: гарантию «распакованный размер равен размеру файла» даёт
/// ПИСАТЕЛЬ (store-only), а импорт кладёт чужой файл байт в байт и содержимое намеренно не
/// разбирает. У импортированного сжатого бандла запись занимает в файле меньше, чем весит сам
/// PNG, и утверждать обратное значило бы называть вес, которого файл не занимает.
///
/// Значение при этом стало НАДЁЖНЕЕ, а не только честнее описанным: раньше это была цифра из
/// оглавления архива, которую никто не сверял с действительностью; теперь она ограничена
/// <see cref="MacroBundleFormat.MaxEntryBytes"/>, а чтение той же записи отказывается отдать
/// байты, если распаковка даёт не объявленный размер.
/// </param>
