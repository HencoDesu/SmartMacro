...
public enum VariableSlot
{
    ...
    // Слота «в имени макроса» здесь больше нет: с волны F4 нода вызова несёт Guid под-макроса, а
    // не строку с подстановками, — собрать guid из переменной невозможно, и слот описывал бы
    // чтение, которого исполнитель не делает.
}
...
///
/// <b>Разбирает ОДИН граф.</b> Под-макрос — тоже граф, поэтому его разбирают отдельным вызовом, и
/// это не упущение: подпрогон получает копию переменных, так что «кто пишет» у родителя и у
/// под-макроса — действительно два разных множества. Сшивает их валидатор
/// (<c>MacroGraphValidator.ValidateBundle</c>) ровно там, где эта раздельность становится
/// ловушкой: под-макрос пишет, родитель читает, значение не возвращается.
///
/// <b>Чистая функция и намеренно в Shared.</b> Ей нужна только модель и больше ничего: ни
...
public static class MacroVariableAnalysis
{
    ...
    private static void Collect(MacroNode node, int order, Dictionary<string, Entry> found)
    {
        switch (node)
        {
            ...
            default:
                // KeyPressNode, DelayNode и RunSubmacroNode переменных не касаются. НОВЫЙ тип
                // ноды попадёт сюда молча — единственное, о чём стоит помнить, когда каталог
                // разрастётся.
                break;
        }
    }
