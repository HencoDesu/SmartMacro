...
public static class IpcMessageTypes
{
    ...
    /// <summary>
    /// Запрос: <see cref="GetTemplatesRequest"/> → Ответ: <c>TemplateDto[]</c>. Шаблоны ОДНОГО
    /// макроса — то, что лежит в <c>templates/</c> внутри его бандла <c>.hsm</c>: одиночные
    /// шаблоны из корня, затем содержимое каждой подпапки-набора.
    ///
    /// <b>Раньше запрос был без нагрузки</b> и отдавал общее дерево <c>templates/</c> в корне
    /// установки. Дерева нет с волны F2: шаблон принадлежит ровно одному макросу, поэтому и
    /// вопрос теперь задают про макрос. Режим «Шаблоны» вместе с этим исчез из рейки, а браузер
    /// переехал внутрь редактора макроса — рейка про сущности, а шаблон перестал быть
    /// самостоятельной сущностью.
    ///
    /// <b>Одни метаданные, ни одного пикселя</b> — и это не экономия, а разделение
    /// обязанностей. Обоснование записано у <see cref="Dto.TemplateDto"/>; коротко: список
    /// маленький и приезжает целиком при открытии макроса, картинки большие и приезжают по одной
    /// за выделением.
    ///
    /// Читается из бандла на каждый запрос, а не из кэша исполнителя: это ВТОРАЯ дорога чтения, и
    /// она обязана показывать то, что лежит в файле сейчас. Кэш (<c>MacroTemplateCache</c>) —
    /// первая, он наполняется целым бандлом на первый прогон и сбрасывается по
    /// <see cref="MacrosChanged"/>.
    ///
    /// ⚠️ В F3 этот запрос исчезнет вовсе: панель начнёт читать <c>.hsm</c> сама. Сейчас файлами
    /// владеет демон, и границу волн размывать не надо.
    /// </summary>
    public const string GetTemplates = "GetTemplates";

    /// <summary>
    /// Запрос: <see cref="GetTemplateImageRequest"/> → Ответ: <c>TemplateImageDto</c>. Байты
    /// ОДНОГО шаблона из бандла одного макроса — для превью.
    ///
    /// Отвергается, когда такого шаблона нет, когда в имени есть сегменты пути (это выход за
    /// <c>templates/</c>, а не имя шаблона) и когда файл крупнее
    /// <see cref="Dto.TemplateLimits.MaxImageBytes"/>. Последнее панель обычно предотвращает
    /// сама — размер она знает из <see cref="GetTemplates"/>, — но проверяют оба конца:
    /// между перечислением и запросом бандл мог смениться.
    /// </summary>
    public const string GetTemplateImage = "GetTemplateImage";

    /// <summary>
    /// Запрос: <see cref="AddMacroTemplateRequest"/> → Ответ: <c>TemplateDto[]</c> (новый перечень
    /// шаблонов этого макроса). Кладёт PNG в бандл, заменяя одноимённый.
    ///
    /// <b>Появился в F2 по необходимости, а не по замыслу.</b> Дерева <c>templates/</c>, куда
    /// пользователь ронял файлы проводником, больше нет; бандл — это zip, и написать в него
    /// панель пока не может: авторство инвертируется только в F3. Значит, единственный способ
    /// положить шаблон в макрос — попросить того, кто владеет файлом. Вместе с
    /// <see cref="DeleteMacroTemplate"/> и <see cref="GetTemplates"/> этот запрос в F3 исчезнет.
    ///
    /// Бандл при этом переписывается ЦЕЛИКОМ — иначе не выйдет атомарной замены, — так что
    /// добавление шаблона стоит ровно столько же, сколько сохранение макроса, и точно так же
    /// поднимает <see cref="MacrosChanged"/>.
    /// </summary>
    public const string AddMacroTemplate = "AddMacroTemplate";

    /// <summary>
    /// Запрос: <see cref="DeleteMacroTemplateRequest"/> → Ответ: <c>TemplateDto[]</c>. Убирает
    /// шаблон из бандла. Удалить несуществующий — не ошибка, а пустая операция.
    /// </summary>
    public const string DeleteMacroTemplate = "DeleteMacroTemplate";
    ...
    /// <see cref="SettingsChanged"/> при этом ЕСТЬ — в отличие от <see cref="GetTemplates"/>,
    /// где его нет: настройки меняет не только пользователь панели, но и наблюдатель за файлом,
    /// то есть правка блокнотом обязана доехать до открытого экрана сама.
    /// </summary>
    ...
    /// Договорённость дословно та же, что у <see cref="SaveMacro"/>, и по той же причине: одно
    /// поле ответа с одним смыслом читается однозначно.
    ///
