...
// RunningMacroDto[], HotkeyFailureDto[]) или голый скаляр (DumpCaptures → string),
// сериализуются как есть, а запросы и события без аргументов нагрузки не несут вовсе.
//
// Нагрузок про САМ МАКРОС здесь больше нет (F3): ни графа, ни имени файла, ни байтов шаблона.
// Библиотекой владеет панель и правит её файлами; по трубе ходят только просьбы к демону —
// запустить, остановить, приостановить хоткеи, поставить точку останова.
...
/// ОДНОГО ГРАФА целиком: редактор всегда знает все свои точки останова, поэтому полная
/// замена ценой лишних нескольких байт убирает целый класс ошибок вида «add и remove пришли не
/// в том порядке».
/// </summary>
/// <param name="MacroName">Макрос (бандл). Он может ещё не существовать — точки может нести несохранённый черновик.</param>
/// <param name="NodeIds">
/// Ноды, на которых обход должен останавливаться. ПУСТОЙ массив снимает у этого графа все точки
...
/// Под-макрос, которому принадлежат ноды, либо <c>null</c> — граф верхнего уровня (волна F4).
/// «Заменить целиком» без этой координаты снимало бы точки родителя всякий раз, когда панель
/// присылает точки его функции.
/// </param>
