...
/// применение — это значение в ключе реестра и запрос UAC, а не поле, которое кто-то перечитает.
///
/// Две галочки, и каждая отвечает ровно за одно:
/// <list type="table">
///   <item><term><see cref="RunAtLogon"/></term><description>значение в ключе <c>Run</c> в <c>HKCU</c></description></item>
///   <item><term><see cref="RunElevated"/></term><description>демон проверяет повышение при старте и перезапускает себя через <c>runas</c></description></item>
/// </list>
///
/// Так было не всегда: над теми же галочками стояло ТРИ механизма, между которыми выбирало
/// приложение, и пара «✓ / ✓» регистрировалась задачей в Планировщике с наивысшими правами,
/// потому что ключ <c>Run</c> не умеет поднимать процесс с повышением. Планировщик убран, цена
/// названа вслух: с обеими галочками окно UAC появится при каждом входе в систему.
/// </summary>
public sealed record StartupSettings
{
    ...
    ///
    /// Умолчание <c>true</c> намеренно: ради elevated-клиентов права и нужны, а свежая установка
    /// обязана вести себя как та, что была до смены манифеста демона на <c>asInvoker</c>, — то
    /// есть спросить UAC.
    /// </summary>
