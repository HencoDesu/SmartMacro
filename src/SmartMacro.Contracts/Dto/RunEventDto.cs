...
/// <c>RunSubmacroNode</c> с селектором по тегам порождает по обходу на каждое подходящее окно, и
/// все они делят <see cref="RunId"/> родителя. Ровно этот случай канве и приходится
/// различать: десять окон, загружающихся через один под-макрос, — это десять обходов
...
/// МАКРОС (бандл), которому принадлежит обход, — и у подобходов тоже. С волны F4 подобход идёт
/// по под-макросу того же бандла, а не по соседнему макросу библиотеки, поэтому «имя графа» и
/// «имя макроса» перестали совпадать: подпись функции живёт отдельно, в
/// <paramref name="SubmacroName"/>. Панель отбирает обходы открытого макроса именно по этому полю.
/// </param>
...
/// <param name="Depth">Вложенность: 0 у обхода, начатого триггером, 1 у обхода его под-макроса. Больше не бывает — под-макросы плоские.</param>
...
/// Под-макрос, который обходят, либо <c>null</c> — граф верхнего уровня. Он же третья координата
/// подсветки: канва зажигает коробку, только когда открыт ТОТ ЖЕ граф.
/// </param>
/// <param name="SubmacroName">
/// Его подпись либо <c>null</c>. Едет по проводу, а не резолвится панелью, по той же причине, что
/// и <see cref="RunEventDto.NodeName"/>: открытый макрос может быть другим или грязным, а
/// переключатель прогонов обязан назвать то, что демон исполняет на самом деле.
///
/// <b>Без неё переключатель врал бы.</b> Обход функции и обход родителя приходят с одним и тем же
/// дескриптором окна, то есть подписываются в чипе одинаково — «0x140804» и «0x140804».
/// </param>
