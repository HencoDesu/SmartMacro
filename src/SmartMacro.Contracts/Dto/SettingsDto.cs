...
/// <param name="FileFault">
/// Почему файл настроек НЕ прочитан, — или <c>null</c>, если <see cref="Settings"/> и есть его
/// содержимое.
///
/// <b>Единственное поле снимка, которое существует ради потери данных.</b> Хранилище нечитаемый
/// файл не переписывает — это его инвариант, — но без этого поля панель показывала бы умолчания
/// (или последний удачный снимок) как содержимое файла, и следующее «Применить» уничтожало бы
/// файл, чинившийся одной запятой. Панель обязана сказать об этом ПРЯМО НАД ФОРМОЙ, а не спрятать
/// в подсказку: человек читает предупреждение ровно там, где собирается нажать кнопку.
///
/// Блокировать «Применить» при этом НЕ надо: перезаписать сломанный файл — законное намерение.
/// </param>
