...
/// следующий запрос, поэтому <c>StopMacro</c> (который ждёт подтверждения от исполнителя)
/// возвращается тогда, когда возвращается, —
/// возможно, много позже отправленных после него запросов. Поэтому каждый ответ сопоставляется
...
public sealed class IpcClient : IIpcClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var connection = _connection
                         ?? throw new IpcRequestException(type, Strings.Dialog_Ipc_NotConnected);
        ...
        try
        {
            ...
            return response.Ok
                ? response
                : throw new IpcRequestException(type, response.Error ?? Strings.Dialog_Ipc_RejectedSilently);
        }
        catch (Exception ex) when (ex is IOException or ObjectDisposedException)
        {
            // Труба порвалась прямо под записью. Поддерживающий цикл это заметит и переподключится.
            throw new IpcRequestException(type, Strings.Dialog_Ipc_Broken, ex);
        }
        finally
        {
    ...
    private async Task DropAsync(IpcConnection? connection, bool notify)
    {
        ...
        foreach (var id in _pending.Keys)
        {
            if (_pending.TryRemove(id, out var completion))
            {
                completion.TrySetException(new IpcRequestException("(соединение)", Strings.Dialog_Ipc_Disconnected));
            }
        }
