...
/// <summary>
/// Цвет для одной строки диагностики среды: опасность у провала, янтарь у предупреждения,
/// приглушённый нейтральный у порядка.
///
/// <c>ConverterParameter</c> выбирает роль: <c>text</c> (по умолчанию) — цвет надписи,
/// <c>border</c> — цвет кромки карточки, <c>surface</c> — её заливка. Три роли одним конвертером,
/// потому что решение у них одно и то же и разъезжаться им нельзя: карточка с красной кромкой и
/// янтарным текстом читалась бы как два разных вердикта об одной проверке.
///
/// У состояния «в порядке» кромки и заливки НЕТ (прозрачные) — такие строки рисуются компактной
/// строкой без карточки. Успешных проверок всегда большинство, и оформи их так же, как провал,
/// провал перестанет бросаться в глаза, то есть блок потеряет единственный смысл.
/// </summary>
public sealed class DiagnosticStatusToBrushConverter : IValueConverter
{
    ...
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (value is not DiagnosticStatus status)
        {
            return null;
        }

        return (parameter as string ?? "text") switch
        {
            "border" => status switch
            {
                DiagnosticStatus.Failed => NocturneBrushes.Get("NocturneDangerBorderBrush", 0xFF5C3A40),
                DiagnosticStatus.Warning => NocturneBrushes.Get("NocturneWarningBorderBrush", 0xFF5C4F33),
                _ => NocturneBrushes.Get("NocturneDividerBrush", 0xFF262939),
            },
            "surface" => status switch
            {
                DiagnosticStatus.Failed => NocturneBrushes.Get("NocturneDangerSurfaceBrush", 0x14DD8189),
                DiagnosticStatus.Warning => NocturneBrushes.Get("NocturneWarningSurfaceBrush", 0x1FDBB277),
                _ => NocturneBrushes.Get("NocturneSurfaceRaisedBrush", 0xFF191B28),
            },
            _ => status switch
            {
                DiagnosticStatus.Failed => NocturneBrushes.Get("NocturneDangerBrush", 0xFFDD8189),
                DiagnosticStatus.Warning => NocturneBrushes.Get("NocturneWarningBrush", 0xFFDBB277),
                _ => NocturneBrushes.Get("NocturneNeutral300Brush", 0xFFCFD3E5),
            },
        };
    }
