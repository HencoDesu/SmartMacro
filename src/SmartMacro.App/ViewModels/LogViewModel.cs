...
public sealed class LogViewModel : ObservableObject, IDisposable
{
    ...
    [
        new(LogLevelDto.Verbose, Strings_App.Log_Level_All),
        new(LogLevelDto.Debug, Strings_App.Log_Level_Debug),
        new(LogLevelDto.Information, Strings_App.Log_Level_Information),
        new(LogLevelDto.Warning, Strings_App.Log_Level_Warning),
        new(LogLevelDto.Error, Strings_App.Log_Level_Error),
    ];
    ...
    public string GapText =>
        string.Format(CultureInfo.CurrentCulture, Strings_App.Log_Gap_Count, _dropped);
    ...
    public string EmptyHint => WasCleared
        ? Strings_App.Log_Cleared_Hint
        : Strings_App.Log_Empty_Hint;
    ...
    public string SummaryText
    {
        get
        {
            if (_all.Count == 0)
            {
                return Strings_App.Log_Header_Empty;
            }

            var text = EntriesWord(_all.Count);
            if (Rows.Count != _all.Count)
            {
                text += string.Format(CultureInfo.CurrentCulture, Strings_App.Log_Header_Shown, Rows.Count);
            }

            return _problems == 0
                ? text
                : text + string.Format(CultureInfo.CurrentCulture, Strings_App.Log_Header_Problems, _problems);
        }
    }
    ...
    private static string EntriesWord(int count) => PluralForms.Format(
        count,
        Strings_App.Log_Header_Entries_One,
        Strings_App.Log_Header_Entries_Few,
        Strings_App.Log_Header_Entries_Many);
}
