...
public sealed class TemplateRowViewModel : ObservableObject
{
        ...
        IReadOnlyList<TemplateReference> usedBy)
    {
        ...
        SizeText = IsDecodable
            ...
            : Strings_App.Editor_Templates_NotPng;
    ...
    public string UsageText => UsedBy.Count == 0 ? Strings_App.Editor_Templates_Unused : Plural(UsedBy.Count);
    ...
    internal static string Plural(int nodes) => PluralForms.Format(
        nodes,
        Strings_App.Editor_Templates_UsedBy_One,
        Strings_App.Editor_Templates_UsedBy_Few,
        Strings_App.Editor_Templates_UsedBy_Many);
}
...
public sealed class TemplateGroupViewModel
{
    internal TemplateGroupViewModel(string? set, IReadOnlyList<TemplateRowViewModel> rows)
    {
        ...
        Title = set ?? Strings_App.Editor_Templates_GroupSingles;
...
public sealed class TemplatesViewModel : ObservableObject, IDisposable
{
    ...
    public string SelectedUsageHeader => _selected is null
        ...
        : SelectedUsages.Count == 0
            ? Strings_App.Editor_Templates_NoUsages
            : string.Format(CultureInfo.CurrentCulture, Strings_App.Editor_Templates_Usages,
    ...
    public bool Import(string fileName, byte[] png)
    {
        ...
        if (string.IsNullOrWhiteSpace(name))
        {
            ImportProblem = string.Format(CultureInfo.CurrentCulture,
                Strings_App.Editor_Templates_BadName, fileName);
            return false;
        }
        ...
        try
        {
            ...
            {
                ImportProblem = string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Templates_AddFailedUnreadable, name);
                return false;
            }
        }
        ...
        {
            ImportProblem = string.Format(CultureInfo.CurrentCulture,
                Strings_App.Editor_Templates_AddFailed, ex.Message);
            return false;
        }
    ...
    public void Delete(TemplateRowViewModel row)
    {
        ...
        {
            ImportProblem = string.Format(CultureInfo.CurrentCulture,
                Strings_App.Editor_Templates_DeleteFailed, ex.Message);
            return;
        }
    ...
    private void LoadPreview(TemplateRowViewModel? row)
    {
        ...
        if (!row.IsDecodable)
        {
            PreviewProblem = Strings_App.Editor_Templates_PreviewNotPng;
            return;
        }
        ...
        if (row.Bytes > MaxPreviewBytes)
        {
            ...
            PreviewProblem =
                string.Format(
                    ...
                    Strings_App.Editor_Templates_PreviewTooBig,
                    row.BytesText,
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            ...
            PreviewProblem = string.Format(CultureInfo.CurrentCulture,
                Strings_App.Editor_Templates_PreviewFailed, ex.Message);
            return;
        }
        ...
        if (png is null)
        {
            ...
            PreviewProblem = Strings_App.Editor_Templates_PreviewGone;
            return;
        }
...
internal static class TemplateFormat
{
    public static string Bytes(long bytes) => bytes < 1024
        ? string.Format(CultureInfo.CurrentCulture, Strings_App.Editor_Templates_Bytes, bytes)
        : bytes < 1024 * 1024
            ? string.Format(
                CultureInfo.CurrentCulture,
                Strings_App.Editor_Templates_Kilobytes,
                (bytes / 1024.0).ToString("0.#", CultureInfo.CurrentCulture))
            : string.Format(
                CultureInfo.CurrentCulture,
                Strings_App.Editor_Templates_Megabytes,
