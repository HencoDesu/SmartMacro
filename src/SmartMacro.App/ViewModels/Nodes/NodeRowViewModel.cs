...
public sealed class NodeEdgeViewModel : ObservableObject
{
    ...
    public string BoxLabel => IsEnd
        ? string.Format(CultureInfo.CurrentCulture, Strings_App.Node_Edge_BoxLabelEnd, ShortLabel)
        : ShortLabel;
        ...
        foreach (var (label, text) in new[] { ("X", _xText), ("Y", _yText), ("W", _widthText), ("H", _heightText) })
        {
            if (NodeInput.ParseInt(text) is null)
            {
                yield return string.Format(
                    CultureInfo.CurrentCulture, Strings_App.Node_Error_Region,
                    nodeName, label, text);
            }
        }
    }
...
public abstract class NodeRowViewModel : ObservableObject
{
    ...
    [
        new(MacroNodeKind.KeyPress, Strings_App.Node_Type_KeyPress),
        new(MacroNodeKind.Click, Strings_App.Node_Type_Click),
        new(MacroNodeKind.Delay, Strings_App.Node_Type_Delay),
        new(MacroNodeKind.AddTag, Strings_App.Node_Type_AddTag),
        new(MacroNodeKind.RemoveTag, Strings_App.Node_Type_RemoveTag),
        new(MacroNodeKind.SetIcon, Strings_App.Node_Type_SetIcon),
        new(MacroNodeKind.RunSubmacro, Strings_App.Node_Type_RunSubmacroMenu),
        new(MacroNodeKind.FindElement, Strings_App.Node_Type_FindElement),
        new(MacroNodeKind.WaitForElement, Strings_App.Node_Type_WaitForElement),
        new(MacroNodeKind.RecognizeTag, Strings_App.Node_Type_RecognizeTag),
    ];
}
