...
public sealed class TargetSelectorViewModel : ObservableObject
{
    ...
    public string Summary
    {
        get
        {
            if (!_useSelector)
            {
                return Strings_App.Editor_Targets_TextContext;
            }
            ...
            {
                (0, 0) => Strings_App.Editor_Targets_TextAll,
                (_, 0) => require,
                (0, _) => string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Targets_TextExcludeOnly, exclude),
                _ => string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Targets_TextRequireExclude, require, exclude),
            };
        }
    }
    ...
    public string BadgeText
    {
        get
        {
            if (!_useSelector)
            {
                return Strings_App.Editor_Targets_CountContext;
            }

            if (TotalCount == 0)
            {
                return Strings_App.Editor_Targets_CountNoWindows;
            }
            ...
            return (require.Length, exclude.Length) switch
            {
                ...
                (_, 0) => string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Targets_CountRequire, count, require),
                (0, _) => string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Targets_CountExclude, count, exclude),
                _ => string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Targets_CountRequireExclude, count, require, exclude),
            };
        }
    }
    ...
    public string BadgeCountText
    {
        get
        {
            if (!_useSelector)
            {
                return Strings_App.Editor_Targets_CompactContext;
            }
    ...
    public string HitText => TotalCount == 0
        ? Strings_App.Editor_Targets_HitsNoWindows
        : string.Format(CultureInfo.CurrentCulture, Strings_App.Editor_Targets_HitsCount,
    ...
    private void Recompute()
    {
        ...
        UntaggedText = untaggedMisses > 0
            ? string.Format(CultureInfo.CurrentCulture, Strings_App.Editor_Targets_Untagged, untaggedMisses)
            : string.Empty;
    ...
    private static string WindowCount(int count) => PluralForms.Format(
        count,
        Strings_App.Editor_Targets_WindowCount_One,
        Strings_App.Editor_Targets_WindowCount_Few,
        Strings_App.Editor_Targets_WindowCount_Many);
}
