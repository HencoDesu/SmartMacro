...
public partial class MacrosView : UserControl
{
    ...
    private async void OnImportMacroClicked(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                Title = Strings_App.Dialog_ImportMacro_Title,
                AllowMultiple = true,
                FileTypeFilter = [new FilePickerFileType(Strings_App.Dialog_MacroFileType) { Patterns = ["*.hsm"] }],
            });
        }
        catch (Exception ex)
        {
        ...
        foreach (var file in picked)
        {
            ...
            else
            {
                vm.ErrorMessage = string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Status_ImportNotOnDisk, file.Name);
            }
        }
    }
    ...
    private async void OnExportMacroClicked(object? sender, RoutedEventArgs e)
    {
        ...
        {
            vm.ErrorMessage = string.Format(CultureInfo.CurrentCulture,
                Strings_App.Editor_Status_ExportFileMissing, item.Name);
            return;
        }
        ...
        try
        {
            ...
            {
                Title = Strings_App.Dialog_ExportMacro_Title,
                SuggestedFileName = item.Name + ".hsm",
                DefaultExtension = "hsm",
                FileTypeChoices = [new FilePickerFileType(Strings_App.Dialog_MacroFileType) { Patterns = ["*.hsm"] }],
            });
            ...
            if (target?.TryGetLocalPath() is { Length: > 0 } destination)
            {
                ...
                vm.StatusMessage = string.Format(CultureInfo.CurrentCulture,
                    Strings_App.Editor_Status_Exported, item.Name);
            }
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            vm.ErrorMessage = string.Format(CultureInfo.CurrentCulture,
                Strings_App.Editor_Status_ExportFailed, ex.Message);
    ...
    private async void OnAddTemplateClicked(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            picked = await top.StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
            {
                Title = Strings_App.Dialog_ImportTemplate_Title,
                AllowMultiple = true,
    ...
    private void OnOpenFolderClicked(object? sender, RoutedEventArgs e)
    {
        ...
        {
            vm.ErrorMessage = string.Format(CultureInfo.CurrentCulture,
                Strings_App.Editor_Status_OpenFolderFailed, ex.Message);
        }
    }
