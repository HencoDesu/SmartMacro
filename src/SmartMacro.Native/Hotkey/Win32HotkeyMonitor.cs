...
//
// Подписчики зовутся В ПУЛЕ ПОТОКОВ, а не в потоке цикла сообщений, — см. RaiseHotkeyPressed.
// Здесь у этого нет той же цены, что у мышиного хука (общесистемного ввода этот поток не держит),
// но последствия всё равно не местные: пока обработчик на стеке, поток не разбирает свою очередь,
// а через неё приходят и следующие WM_HOTKEY, и WM_QUIT из StopAsync — то есть приостановка
// хоткеев, перерегистрация по изменению библиотеки и выключение демона встали бы в очередь за
// прогоном макроса. Правило у двух мониторов одно, и держать его написанным только в одном из
// близнецов — верный способ потерять его в обоих.
...
public sealed partial class Win32HotkeyMonitor : IDisposable
{
    ...
    /// <summary>
    /// Поднимается, когда срабатывает любая зарегистрированная горячая клавиша. Подписчики
    /// получают <c>Id</c> привязки (то самое значение, которое передали при регистрации) и сами
    /// отвечают за то, чтобы сопоставить его со смыслом.
    ///
    /// <b>Поднимается в потоке ПУЛА, а не в потоке цикла сообщений</b> — см.
    /// <see cref="RaiseHotkeyPressed"/>. Порядок между двумя быстрыми нажатиями поэтому не
    /// гарантирован; ждать его было и нечего — прогон макроса всё равно асинхронный.
    /// </summary>
    ...
    ///
    /// Близнец <c>Win32MouseHookMonitor.RaiseHotkeyPressed</c>, и довод общий: дальше по цепочке
    /// синхронно идёт весь запуск макроса — слушатель, оркестратор, веер по целевым окнам и
    /// блокирующие <c>SendMessage(WM_ACTIVATEAPP)</c> в замороженные клиенты игры. У мышиного
    /// хука цена этому — снятый Windows хук; здесь она мягче, но не местная: этот же поток
    /// разбирает очередь, через которую приходят следующие WM_HOTKEY и WM_QUIT из
    /// <see cref="StopAsync"/>, так что приостановка хоткеев, перерегистрация по изменению
    /// библиотеки и выключение демона встали бы в очередь за прогоном макроса. Лечить один
    /// монитор и не лечить второй — это ровно тот способ терять правила, из-за которого дефект и
    /// завёлся.
    ///
    /// Ловим внутри задания: исключение подписчика на потоке пула иначе стало бы необработанным.
    /// </summary>
    ...
    /// <summary>
    /// Шов для тестов: то же, что делает цикл сообщений, получив <c>WM_HOTKEY</c>. Настоящий
    /// <c>RegisterHotKey</c> в тесте не нажать, а проверять здесь есть что — подписчик обязан
    /// оказаться не на этом потоке.
    /// </summary>
    internal void InvokeHotkeyForTests(int id) => RaiseHotkeyPressed(id);
