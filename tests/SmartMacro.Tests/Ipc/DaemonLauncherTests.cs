...
public class DaemonLauncherTests
{
    ...
    {
        // Оба рядом в одной выходной папке — это раскладка поставки, и она не имеет права
        // превращаться в поиск по файловой системе: запустить какой-то посторонний
        // SmartMacro.Daemon.exe, попавшийся под руку, хуже, чем сказать «не нашли».
