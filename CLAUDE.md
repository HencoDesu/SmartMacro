...
**Nothing is in flight.** The rename to SmartMacro, the node-graph macro model, tags replacing the character roster, the daemon/panel split, the whole UI, the debugger, the settings store and the shipped layout are all built and green. `docs/refactoring-plan-split.md` and `docs/design/implementation-plan.md` are the roadmaps that got us here and are now **history, not instructions** — they describe intent at the time, and several of their claims were disproved on contact (the targets badge needed no new IPC; the breakpoint dot could not live on the box corner). Read them for context, never as a to-do.
...
**There is no migration and none is coming.** A file from before this change fails to parse, and `MacroGraphStore` renames it to `*.json.incompatible` and says so loudly rather than skipping it in silence — an empty library with no explanation is the failure mode that costs an afternoon. Only *parse* failures move; a locked or unreadable file is left alone, because that is temporary. `MacroGraphJson` uses `UnsafeRelaxedJsonEscaping`, so Cyrillic in `macros/*.json` reads as «Лучник» and the files diff like text.
