...
**Still ahead:** W0.4 — the canvas editor. It is the only editor the plan allows: the rows editor in `MacrosDialog` was rejected on design review and is meant to be deleted, not extended. `docs/design/implementation-plan.md` breaks the approved mockup into waves D1–D5 and is explicit about which parts need engine work first (debugger, run-event stream over IPC, variable analysis) — none of that exists yet. Stage 5 (translating comments to Russian) is deliberately last.
