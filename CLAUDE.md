...
Gate: `dotnet run --project tests/SmartMacro.Tests` — **843 tests**, and they are expected green before anything is committed.
...
**Autostart is the only part applied outside the process**, and it is two checkboxes over three mechanisms, the app choosing: nothing / `Run` key in `HKCU` / (no autostart, elevation requested at manual start) / **Scheduled Task with highest privileges** — because the `Run` key cannot elevate. Reconciliation is always FULL, including removing what should not be there, or switching ✓/— → ✓/✓ would leave both and the daemon would launch twice. `StartupReconciler` also reconciles at startup, not only on edit, so an install whose file says "run at logon" but whose registry entry was swept away fixes itself. ⚠️ **Registration can FAIL** (only an administrator can create that task), and **the only place that says so is the log again** — which is exactly the D4 hotkey defect: box ticked, clean UI, nothing happens. The diagnostic that closed it was deleted with the rest of them; this is the sharpest edge of that decision and the owner took it knowingly.
